...
console.log("=== Datenschutzprüfung mit Wortgrenzen ===");
const treffer = m0.personen.filter(p => new RegExp(`\\b${p.nachname}\\b`).test(txt) || new RegExp(`\\b${p.vorname}\\b`).test(txt));
console.log("echte Namenstreffer:", treffer.length, treffer.map(p=>p.nachname).join(", ") || "(keine)");
// Auch E-Mails und IDs prüfen
const mails = m0.personen.filter(p => txt.includes(p.email)).length;
const ids = m0.personen.filter(p => txt.includes(p.id)).length;
console.log("E-Mail-Adressen:", mails, "| Personen-IDs:", ids);
console.log(treffer.length===0 && mails===0 && ids===0 ? "→ BESTANDEN: keine personenbezogenen Daten" : "→ FEHLER");

const rg = baueRechnung(db, m0, heute().slice(0,7));
const bytes = rechnungPDF(rg);
fs.writeFileSync('/tmp/r.pdf', Buffer.from(bytes));
fs.writeFileSync('/tmp/r.doc', rechnungHTML(rg));
console.log("\n=== Dateien ===");
console.log("PDF", bytes.length, "Bytes | Word", rechnungHTML(rg).length, "Bytes");
