...
const db = startbestand(); const ym = heute().slice(0,7);
for (const m of db.mandanten) {
  const b = pruefen(m, `${ym}-01`, `${ym}-28`);
  console.log(`\n${m.name}: ${b.length} Befunde`);
  b.slice(0,6).forEach(x=>console.log(`   ${x.datum} [${x.art}] ${x.titel} — ${x.text}`));
  if (b.length) {
    const d = b[0].datum; const bes = besetzung(m, d);
    console.log("   Besetzung an dem Tag:", Object.entries(bes).map(([k,v])=>`${k}:${v.anzahl}/${v.soll}`).join("  "));
    const eF = m.einheiten.find(e=>einheitDienst(m,e.id,d)==="F");
    if (eF) console.log("   Frühgruppe:", eF.name, "| Personen:", aktive(m,d).filter(p=>einheitAm(p,d)===eF.id).length,
      "| abwesend:", aktive(m,d).filter(p=>einheitAm(p,d)===eF.id && abwesenheitAm(m,p.id,d)).length);
  }
}
